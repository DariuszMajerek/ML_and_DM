...
Dlaczego w ogóle zależy nam na podobieństwie w bogatszej przestreni? Ponieważ funkcja decyzyjna kernelowego SVM ma postać $f(x)=\sum_{i \in SV}\alpha_i y_i K(x_i,x)+b$, gdzie każdy wyraz sumy wyraża, jak bardzo nowy punkt $x$ przypomina dany wektor nośny $x_i$ — ale właśnie w przestrzeni przekształconej. Jeśli dwie obserwacje należą do tej samej klasy i leżą blisko siebie w tej przestrzeni, ich jądro będzie duże i wzmocni decyzję modelu. Gdybyśmy mierzyli podobieństwo wyłącznie w oryginalnych cechach, nie bylibyśmy w stanie wykryć struktur nieliniowych, takich jak okręgi czy parabole. Bogatsza przestrzeń sprawia, że punkty, które są „podobne" w sensie nieliniowej granicy decyzyjnej, stają się faktycznie bliskie w sensie iloczynu skalarnego — i właśnie to podobieństwo model wykorzystuje do klasyfikacji.
...
W hard-margin SVM rozwiązujemy zadanie z ograniczeniami wynikającymi z nierówności. Aby przejść do postaci dualnej, konstruujemy Lagrangian przez dołączenie ograniczeń z mnożnikami $\alpha_i\ge 0$. Otrzymujemy funkcję:
...
Ponieważ w problemie dualnym maksymalizujemy tę funkcję względem $\alpha$ , więc mamy postać^[Ograniczenie $\alpha_i \ge 0$ wynika z definicji mnożników Lagrange'a dla ograniczeń nierównościowych: aby Lagrangian był ograniczony od dołu, mnożniki przy ograniczeniach $\ge 0$ muszą być nieujemne. Ograniczenie $\sum_i \alpha_i y_i = 0$ pochodzi z warunku stacjonarności $\partial \mathcal{L}/\partial b = 0$: ponieważ $b$ jest wolnym parametrem, jego pochodna musi znikać, co daje dokładnie tę sumę.]
...
W tym miejscu potrzebna jest jeszcze interpretacja współczynników $\alpha_i$. Sam problem dualny mówi, że każda obserwacja ma swój mnożnik Lagrange'a, ale nie wyjaśnia jeszcze, które obserwacje rzeczywiście wpływają na granicę decyzyjną. Do tego służą warunki KKT (Karush–Kuhn–Tucker), czyli warunki optymalności dla problemów z ograniczeniami.^[Warunki KKT to uogólnienie metody mnożników Lagrange'a na ograniczenia nierównościowe. Składają się z czterech grup: (1) dopuszczalność pierwotna — punkt spełnia ograniczenia; (2) dopuszczalność dualna — mnożniki $\alpha_i \ge 0$; (3) stacjonarność — gradient Lagrangianu znika w punkcie optymalnym; (4) komplementarność — $\alpha_i [y_i(w^\top x_i + b) - 1] = 0$. W SVM warunek (4) jest kluczowy: dzieli obserwacje na nieaktywne ($\alpha_i = 0$, poza marginesem) i wektory nośne ($\alpha_i > 0$, na brzegu marginesu), które jako jedyne wyznaczają położenie hiperpłaszczyzny.]

W przypadku hard-margin SVM najważniejszy jest warunek komplementarności
...
::: {.column-margin}
Skąd pochodzi każdy składnik warunku $C - \alpha_i - \mu_i = 0$:

- $C$ — z kary $C\xi_i$ w funkcji celu,
- $-\alpha_i$ — z ograniczenia marginesowego $y_i(w^\top x_i+b) \ge 1-\xi_i$ (zawiera $\xi_i$ z minusem),
- $-\mu_i$ — z ograniczenia nieujemności $\xi_i \ge 0$.

Ponieważ $\mu_i \ge 0$, z równości $\alpha_i = C - \mu_i$ wynika $\alpha_i \le C$. Łącząc z $\alpha_i \ge 0$ (definicja mnożników Lagrange'a), dostajemy $0 \le \alpha_i \le C$.
:::
...
Jeżeli przestrzeń ta jest bardzo wysokowymiarowa albo nieskończenie wymiarowa, jawna rekonstrukcja $\hat w$ nie jest potrzebna/możliwa. Model jest reprezentowany przez wektory nośne, ich współczynniki $\alpha_i$, etykiety $y_i$, funkcję jądra oraz wyraz wolny $b$.
