...
-   `R2_train` = 0.751, `R2_test` = 0.669 - model liniowy wyjaśnia ok. 75% wariancji zmiennej docelowej na treningu i ok. 67% na teście. Różnica jest zauważalna, ale sam pojedynczy split train/test nie wystarcza jeszcze do stwierdzenia silnego przeuczenia.
...
-   `F_train` = 90.43 przy `p` = 13 i dof = 390 - statystyka $F$ testuje hipotezę zerową: wszystkie współczynniki nachyleń są równe 0 (model nie wnosi nic ponad średnią). Tak wysoka wartość sugeruje, że przy klasycznych założeniach model jako całość jest istotny statystycznie, czyli przynajmniej część cech jest związana z targetem.

Ponieważ między wynikiem treningowym i testowym widać lukę generalizacyjną, sprawdźmy, czy regularyzacja poprawi stabilność modelu.
:::
...
Po standaryzacji „jednostka” każdej cechy jest porównywalna (1 odchylenie standardowe), więc kara na współczynnikach działa symetrycznie. Dlatego w praktyce dla regresji logistycznej, ridge/lasso oraz większości modeli liniowych z regularyzacją standaryzacja jest standardowym i silnie zalecanym elementem pipeline'u, chyba że cechy są już porównywalnie przeskalowane.
...
alpha_grid = np.linspace(0.1, 1.0, 10)    # 0.1, 0.2, ..., 1.0
...
W tym uruchomieniu kalibracja parametrów $\lambda$ i $\alpha$ wskazała rozwiązanie bliskie części ridge'owej, ale nadal wewnątrz rodziny Elastic Net. Wyniki $R^2$ na treningu i teście zmieniły się nieznacznie, więc w tym przykładzie sama regularyzacja nie daje wyraźnej poprawy jakości generalizacji. Na koniec, aby sprawdzić, na ile wnioski zależą od konkretnego podziału danych, porównamy modele OLS i ridge z wykorzystaniem walidacji krzyżowej.
...
Wyniki są niemal identyczne, co sugeruje, że w tym przykładzie regularyzacja nie daje wyraźnej poprawy średniego $R^2$ ani stabilności wyniku.
...
W ujęciu statystycznym „uczenie” parametrów polega na maksymalizacji wiarygodności $L$. Ponieważ iloczyny są niewygodne obliczeniowo, przechodzi się na logarytm wiarygodności:
...
# =========================
# Dataset ma jeden split "train" i target "is_cancer"
...
Uwaga: próg został tutaj dobrany na tych samych predykcjach OOF, na których potem liczymy metryki po kalibracji. Jest to dobre do demonstracji wpływu progu decyzyjnego, ale nie daje w pełni bezstronnej oceny generalizacji po strojeniu progu. Do rzetelnej oceny należy użyć osobnego zbioru walidacyjnego albo zagnieżdżonej walidacji krzyżowej.
:::
...
Historycznie do analizy dyskryminacyjnej dochodzono co najmniej dwiema drogami. Pierwsza (zwykle kojarzona z Fisherem) wynikała z problemu znalezienia kierunku projekcji, na którym klasy są najlepiej rozdzielone w sensie stosunku wariancji „między klasami” do wariancji „wewnątrz klas” [@fisher1936]. Druga droga ma charakter probabilistyczno-decyzyjny: zaczynamy od założeń o rozkładach klas (najczęściej normalnych) i z reguły Bayesa wyprowadzamy regułę klasyfikacji w postaci porównania funkcji punktacji dla klas. Ten drugi sposób jest bliższy temu, jak dziś prezentuje się LDA/QDA w uczeniu maszynowym, czyli jako klasyfikatory generatywne.
...
Parametry $\mu_k$, $\Sigma_k$ oraz $\pi_k$ są nieznane i są uczone na danych treningowych z wykorzystaniem etykiet klas (czyli w pełni nadzorowany sposób). W praktyce: $\pi_k$ estymujemy jako częstość klasy w treningu, $\mu_k$ jako średnią wektora cech w klasie, a $\Sigma$ lub $\Sigma_k$ jako macierze kowariancji (wspólne lub klasowe, zależnie od wariantu).
...
Uwaga praktyczna: spotyka się też wariant pośredni z diagonalnymi macierzami kowariancji. Wersja z diagonalnymi $\Sigma_k$ (osobno w każdej klasie) jest bliska „gaussowskiemu naiwnemu Bayesowi” z normalnymi rozkładami cech, bo zakłada brak korelacji cech wewnątrz klasy.

### Jak estymuje się parametry w uczeniu nadzorowanym?

Dla danych treningowych $\{(x_i,y_i)\}_{i=1}^n$, gdzie $n_k$ to liczba obserwacji w klasie $k$, standardowe estymatory empiryczne mają postać:
...
Dla LDA estymujemy wspólną kowariancję jako kowariancję „wewnątrzklasową” (*pooled covariance*); poniżej zapis w wersji z klasycznym mianownikiem nieobciążonym:
...
Dla QDA estymujemy $\hat{\Sigma}_k$ osobno w każdej klasie, również w wersji z mianownikiem nieobciążonym:
...
::: {#exm-3}
Poniżej przykład LDA i QDA na klasycznym zbiorze Iris dostępnym na Hugging Face (`scikit-learn/iris`). Zbiór zawiera 150 obserwacji trzech gatunków irysów, opisanych czterema cechami liczbowymi.
