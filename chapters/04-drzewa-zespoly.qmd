...
Wyniki porównania wskazują wyraźną hierarchię jakości w tym zadaniu wieloklasowym (klasy gatunków): **XGBoost** osiąga najlepsze dopasowanie (accuracy = 0.7950, balanced_accuracy = 0.7950, F1_macro = 0.7958), następnie **CatBoost** (0.7805 / 0.7805 / 0.7809) i **Random Forest** (0.7705 / 0.7705 / 0.7697). Dalej plasują się **Bagging** (0.7310 / 0.7310 / 0.7314) oraz pojedyncze drzewa (**przycięte**: 0.6930 / 0.6930 / 0.6936, **surowe**: 0.6910 / 0.6910 / 0.6913). Najsłabiej wypadł **AdaBoost** (0.5425 / 0.5425 / 0.5118), co w tym kontekście jest istotną informacją diagnostyczną.

Po pierwsze, w tym konkretnym zbiorze (po wcześniejszej selekcji cech numerycznych) przewagę mają metody oparte o **gradient boosting drzew**. XGBoost wygrywa zwykle wtedy, gdy relacje między cechami a klasami są nieliniowe i wymagają kolejnych korekt na resztach/gradientach, a jednocześnie potrzebna jest skuteczna regularizacja (*shrinkage*, *subsampling*, kary złożoności). W praktyce XGBoost łączy wysoką elastyczność z wieloma mechanizmami kontroli przeuczenia, co często daje najlepszy kompromis *bias–variance*.

CatBoost uzyskał wynik nieco słabszy od XGBoost. Jest to spójne z intuicją: główna przewaga CatBoost ujawnia się w danych z licznymi zmiennymi kategorycznymi (*ordered target statistics*, *ordered boosting*), natomiast w naszym pipeline’ie korzystamy niemal wyłącznie z cech liczbowych. CatBoost pozostaje jednak bardzo konkurencyjny, bo jest silnym wariantem boostingu drzew również dla danych numerycznych, a różnica jakości może wynikać zarówno z doboru hiperparametrów, jak i z tego, że XGBoost w tej konfiguracji lepiej dopasował kompromis między głębokością drzew a regularizacją.

Random Forest jest najlepszą metodą „baggingową” w zestawieniu. Wynik lepszy niż Bagging jest typowy: losowanie cech w węzłach (`max_features`) dodatkowo **dekoruluje drzewa**, co mocniej redukuje wariancję zespołu. Jednocześnie RF z reguły nie „dopycha” biasu tak skutecznie jak boosting, dlatego – przy porównywalnym nakładzie obliczeń – bywa minimalnie słabszy od dobrze nastrojonego gradient boostingu.

Bagging poprawia pojedyncze drzewo głównie przez redukcję wariancji (uśrednianie wielu modeli uczonych na próbkach bootstrapowych). Jest to widoczne w przeskoku z okolic 0.69 (pojedyncze drzewo) do okolic 0.73 (bagging). Różnica między drzewem surowym a przyciętym jest natomiast mała, co sugeruje, że w tej konfiguracji przycinanie redukuje złożoność, ale nie zmienia istotnie granic decyzyjnych w obszarach, które dominują w teście. Z dydaktycznego punktu widzenia to dobry przykład faktu, że **upraszczanie drzewa** często poprawia interpretowalność i stabilność, ale nie gwarantuje zauważalnego wzrostu metryk – szczególnie, gdy problem jest złożony, a pojedyncze drzewo ma ograniczoną moc aproksymacyjną.

Słaby wynik AdaBoost jest w tym zadaniu szczególnie pouczający. W wersji wieloklasowej (oraz przy płytkich *weak learners*) AdaBoost może gorzej radzić sobie, gdy granice klas są złożone, klasy są do siebie podobne w przestrzeni cech, a „trudne” obserwacje (które dostają większe wagi) zawierają szum lub są niejednoznaczne. Wówczas algorytm może nadmiernie koncentrować się na przykładach trudnych do rozdzielenia i tracić jakość globalną (spadek macro-F1 jest tu wyraźny). W praktyce AdaBoost bywa dobry jako prosty baseline, ale w zadaniach tablicowych częściej ustępuje nowocześniejszym wariantom gradient boostingu.
